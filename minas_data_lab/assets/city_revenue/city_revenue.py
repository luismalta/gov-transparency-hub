...
class CityRevenueConfig(Config):
    city: str
...
def city_revenue(context, config: CityRevenueConfig) -> Output[pd.DataFrame]:
    month = datetime.now().month - 1
    payload = ['INT_EXR=2023', 'CHAR_ID_EMP=1', 'LG_ALT_PAG=S', f'INT_MES_INI={month}', f'INT_MES_FIM={month}', 'URL=Tempo_Real_Receitas']
    scrapper = PortalTransparenciaScrapper(config.city)
...
def city_revenue_cleaned(context, city_revenue: pd.DataFrame) -> Output[pd.DataFrame]:
    ...
    # FIXME: Handle empty result on first run
