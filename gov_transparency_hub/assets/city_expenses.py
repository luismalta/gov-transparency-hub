...
@asset(partitions_def=daily_city_partition, group_name="expense")
def city_expense_file(context, s3_resource: S3Resource):
    ...
    object_name = format_object_name(
        "summarized_expense", bucket_name, partition_date_str
    )
    s3_resource.upload_object(bucket_name, object_name, city_revenue_df)


@asset(
    deps=["city_expense_file"], partitions_def=daily_city_partition, group_name="expense"
)
def city_expense(context, s3_resource: S3Resource, postgres_resource: PostgresResource):
    ...
    city_expense.query(
        'not Credor.str.contains("Clique na lupa para mais informações")',
        engine="python",
        inplace=True,
    )
    ...
    if detailed_expenses_list:
        ...
        detailed_expenses_df["effort_date"] = pd.to_datetime(
            detailed_expenses_df["effort_date"], format="%d/%m/%Y"
        )
        detailed_expenses_df["settlement_date"] = pd.to_datetime(
            detailed_expenses_df["settlement_date"], format="%d/%m/%Y"
        )
        detailed_expenses_df["payment_date"] = pd.to_datetime(
            detailed_expenses_df["payment_date"], format="%d/%m/%Y"
        )

        detailed_expenses_df["value"] = detailed_expenses_df["value"].replace(
            r"\.", "", regex=True
        )
        detailed_expenses_df["value"] = detailed_expenses_df["value"].replace(
            r",", ".", regex=True
